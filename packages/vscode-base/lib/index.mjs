...
class RemoteAuthoritiesImpl {
    ...
    rewrite(uri) {
        ...
        return URI.from({
            scheme: isWeb ? this._preferredWebSchema : Schemas.qqmusicRemoteResource,
            authority: `${host}:${port}`,
...
class FileAccessImpl {
    ...
    asBrowserUri(uriOrModule, moduleIdToUrl, __forceCodeFileUri) {
        ...
        if (isNative && __forceCodeFileUri && uri.scheme === Schemas.file) {
            convertToQqmusicFileResource = true;
        }
        // Also convert `file:` URIs in the web worker extension host (running in desktop) case
        if (uri.scheme === Schemas.file && typeof globals.importScripts === 'function' && globals.origin === 'qqmusic-file://qqmusic-app') {
            convertToQqmusicFileResource = true;
        }
...
class SimpleSequence {
    constructor(elements, onDidAdd, onDidRemove) {
        ...
        this.onDidSplice = Event.any(Event.map(onDidAdd, e => ({ start: this.elements.length, deleteCount: 0, toInsert: [e] })), Event.map(Event.filter(Event.map(onDidRemove, e => this.elements.indexOf(e)), i => i > -1), i => ({ start: i, deleteCount: 1, toInsert: [] })));
...
export { ArrayQueue, AsyncEmitter, AsyncReferenceCollection, AutoOpenBarrier, Barrier, CancellationToken, CancellationTokenSource, CharCode, CommandService, CommandsRegistry, ConfigKeysIterator, Constants, Cookie, DebounceEmitter, DeferredPromise, Delayer, Disposable, DisposableStore, Emitter, ErrorHandler, Event, EventBufferer, EventMultiplexer, FileAccess, ICommandService, IInstantiationService, IdleValue, ImmortalReference, InstantiationService, IntervalCounter, IntervalTimer, Iterable, LRUCache, Limiter, LinkedList, LinkedMap, MarshalledId, MultiDisposeError, MutableDisposable, NotImplementedProxy, PathIterator, PauseableEmitter, Platform, PlatformToString, Promises, QMBuffer, Queue, ReadonlyMapView, RefCountedDisposable, ReferenceCollection, Relay, RemoteAuthorities, ResourceMap, ResourceQueue, RunOnceScheduler, RunOnceWorker, Schemas, ScrollState, Scrollable, ScrollbarVisibility, Sequence, Sequencer, SequencerByKey, ServiceCollection, SimpleSequence, SmoothScrollingOperation, SmoothScrollingUpdate, StopWatch, StringIterator, SyncDescriptor, TaskSequentializer, TernarySearchTree, ThrottledDelayer, ThrottledWorker, Throttler, TimeoutTimer, Touch, URI, UriIterator, User, _util, areFunctions, arrayInsert, asArray, asPromise, assertAllDefined, assertIsDefined, assertNever, assertType, basename$1 as basename, binarySearch, bufferToReadable, bufferToStream, bufferedStreamToBuffer, canceled, coalesce, coalesceInPlace, combinedDisposable, commonPrefixLength$1 as commonPrefixLength, consumeReadable, consumeStream, createCancelablePromise, createDecorator, createProxyObject, createSyncDescriptor, delimiter, delta, dirname$1 as dirname, disposableTimeout, dispose, distinct, distinctES6, emptyStream, equals, errorHandler, extname$1 as extname, findFirstInSorted, first, firstOrDefault, firstParallel, flatten, format, fromNow, generateUuid, getAllMethodNames, getAllPropertyNames, getDriveLetter, getOrSet, getRandomElement, getRoot, getSingletonServiceDescriptors, glob, globals, groupBy, hasDriveLetter, illegalState, index, indexOfPath, insert, insertInto, isAbsolute, isAndroid, isArray, isBoolean, isDefined, isDisposable, isEmptyObject, isEqual$1 as isEqual, isEqualOrParent$1 as isEqualOrParent, isFalsyOrEmpty, isFunction, isIOS, isIterable, isLinux, isLinuxSnap, isLittleEndian, isMacintosh, isMobile, isMusic, isNative, isNonEmptyArray, isNumber, isObject, isPathSeparator, isPromiseCanceledError, isReadableBufferedStream, isReadableStream, isRootOrDriveLetter, isString, isStringArray, isThenable, isUNC, isUUID, isUndefined, isUndefinedOrNull, isValidBasename, isWeb, isWindows, isWindowsDriveLetter, join, lastIndex, listenStream, mapArrayOrNot, mapFind, mapToString, markAsSingleton, maxIndex, minIndex, move, newWriteableBufferStream, newWriteableStream, normalize, onUnexpectedError, once, optional, parse$1 as parse, parseLineAndColumnAware, peekReadable, peekStream, platform$1 as platform, posix, prefixedBufferReadable, prefixedBufferStream, prefixedReadable, prefixedStream, process$1 as process, pushToEnd, pushToStart, quickSelect, raceCancellablePromises, raceCancellation, raceTimeout, range, readUInt16LE, readUInt32BE, readUInt32LE, readUInt8, readableToBuffer, refineServiceDecorator, registerSingleton, relative, remove, resolve, resources, retry, runWhenIdle, sanitizeFilePath, sep, sequence, setDisposableTracker, setGlobalLeakWarningThreshold, setImmediate, setToString, setUnexpectedErrorHandler, shuffle, sortedDiff, splice, streamToBuffer, streamToBufferReadableStream, strings, tail, tail2, timeout, toDisposable, toErrorMessage, toLocalISOString, toNamespacedPath, toPosixPath, toReadable, toSlashes, toStream, toUint32, toUint8, top, topAsync, transform, uniqueFilter, uriToFsPath, userAgent, validateConstraint, validateConstraints, win32, withNullAsUndefined, withUndefinedAsNull, writeUInt16LE, writeUInt32BE, writeUInt32LE, writeUInt8 };
